...
interface Mission {
  ...
  budget: string; description: string; company: string; postedAt: string;
}

const MISSIONS: Mission[] = [
  { id: 1, title: 'Refonte Identité Visuelle - Startup Fintech', category: 'Branding', duration: '3-4 semaines', budget: '2 500 €', company: 'LumiPay', postedAt: 'Il y a 2 jours', description: "Nous recherchons un expert en branding pour refondre complètement l'identité visuelle de notre startup fintech." },
  { id: 2, title: 'Landing Page High-Convert - E-commerce', category: 'Web Design', duration: '1-2 semaines', budget: '1 200 €', company: 'EcoShop', postedAt: 'Il y a 5 heures', description: "Besoin d'une landing page optimisée pour la conversion pour le lancement d'un nouveau produit écologique." },
  { id: 3, title: 'Série de Vidéos Motion Design', category: 'Motion Design', duration: '2-3 semaines', budget: '3 000 €', company: 'TechFlow', postedAt: 'Hier', description: 'Création de 5 vidéos en motion design de 15-30 secondes pour une campagne publicitaire.' },
];
...
export default function DashboardPage() {
  ...
  const [activeSettingsTab, setActiveSettingsTab] = useState<'main' | 'profile' | 'security' | 'notifications'>('main');
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.role === 'freelancer') {
        await supabase.from('freelancers').update({ bio: profileForm.bio.trim() }).eq('user_id', profile.user_id);
      }
  ...
  const stats = [
    ...
    { id: 'saved', label: 'Enregistrés', value: savedMissions.length.toString(), color: 'text-yellow-400' },
  ];
    ...
    <div className="bg-[var(--bg-surface)] rounded-2xl md:rounded-3xl border border-[var(--border-color)] p-4 md:p-8">
      ...
      <div className="space-y-4">
        {filteredMissions.length > 0 ? filteredMissions.map(mission => (
          <div key={mission.id} onClick={() => setSelectedMission(mission)}
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
              {appliedMissions.includes(mission.id) ? (
                <span className="text-brand-mint text-sm font-bold flex items-center gap-1"><CheckCircle size={16} /> Postulé</span>
              ) : (
          ...
          <div className="text-center py-12 text-brand-gray">Aucune mission ne correspond à votre recherche.</div>
        )}
      </div>
    </div>
  );
          ...
          <div className="space-y-6">
            {notifications.map(notif => (
              <div key={notif.id} className="flex gap-4">
                ...
                <div>
                  <p className={`text-sm leading-relaxed ${notif.read ? 'text-brand-gray' : 'text-[var(--text-primary)]'}`}>{notif.text}</p>
    ...
    <div className="bg-[var(--bg-surface)] rounded-3xl border border-[var(--border-color)] p-8">
      ...
      {appliedMissions.length > 0 ? (
          ...
          {MISSIONS.filter(m => appliedMissions.includes(m.id)).map(mission => (
            <div key={mission.id} className="flex items-center justify-between p-6 rounded-2xl border border-[var(--border-color)] bg-white/5">
              <div className="flex items-center gap-6">
                <div className="w-12 h-12 rounded-xl bg-brand-mint/10 flex items-center justify-center text-brand-mint font-bold">{mission.company.charAt(0)}</div>
                <div>
        ...
        <div className="text-center py-20">
          ...
          <button onClick={() => setActiveTab('projects')} className="bg-brand-mint text-[#0D1117] px-8 py-3 rounded-xl font-bold hover:scale-105 transition-all">
  ...
  const renderSaved = () => (
    <div className="bg-[var(--bg-surface)] rounded-3xl border border-[var(--border-color)] p-8">
      ...
      {savedMissions.length > 0 ? (
        ...
        <div className="text-center py-20 text-brand-gray">Vous n'avez aucune mission enregistrée.</div>
      )}
    </div>
  );
  ...
  const renderMessages = () => (
    <div className="bg-[var(--bg-surface)] rounded-3xl border border-[var(--border-color)] p-8 flex flex-col items-center justify-center text-center min-h-[500px]">
      ...
      <p className="text-brand-gray text-lg max-w-md mb-10">Pour toute question, discutez directement avec notre équipe via WhatsApp.</p>
      <a href="https://wa.me/22890123456" target="_blank" rel="noopener noreferrer"
        ...
        <WhatsAppIcon size={24} />Ouvrir WhatsApp
      </a>
    </div>
  );
    ...
    <div className="bg-[var(--bg-surface)] rounded-3xl border border-[var(--border-color)] p-8">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl font-bold">
          {activeSettingsTab === 'main' ? t('dashboard.settings.title') : (
            <button onClick={() => setActiveSettingsTab('main')} className="flex items-center gap-2 hover:text-brand-mint transition-colors">
      ...
      {activeSettingsTab === 'main' && (
        <div className="space-y-6">
          <div className="flex items-center gap-6 p-6 rounded-2xl bg-white/5 border border-[var(--border-color)]">
            <div className="w-20 h-20 rounded-full bg-brand-mint flex items-center justify-center text-[#0D1117] text-3xl font-bold">{initials}</div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={item.id} onClick={() => setActiveSettingsTab(item.id as 'profile' | 'security' | 'notifications')}
        ...
        <form className="space-y-6 max-w-xl" onSubmit={handleSaveProfile}>
          ...
          <div className="space-y-2">
            ...
            <textarea value={profileForm.bio} onChange={e => setProfileForm({ ...profileForm, bio: e.target.value })}
              rows={4} placeholder="Décrivez-vous..."
          ...
          {settingsStatus === 'success' && <p className="text-brand-mint text-sm">✅ Profil mis à jour !</p>}
          <div className="flex gap-4">
            ...
            <button type="button" onClick={() => setActiveSettingsTab('main')}
        ...
        <form className="space-y-6 max-w-xl" onSubmit={handlePasswordChange}>
          <div className="space-y-2">
            ...
            <div className="relative">
              <input type={showNewPwd ? 'text' : 'password'} value={passwordForm.new}
              ...
              <button type="button" onClick={() => setShowNewPwd(!showNewPwd)} className="absolute right-4 top-1/2 -translate-y-1/2 text-brand-gray">
          ...
          <div className="space-y-2">
            ...
            <input type={showNewPwd ? 'text' : 'password'} value={passwordForm.confirm}
          ...
          <div className="flex gap-4">
            <button type="submit" disabled={settingsStatus === 'saving'}
              className="bg-brand-mint text-[#0D1117] px-8 py-3 rounded-xl font-bold hover:scale-105 transition-all disabled:opacity-60">
              {settingsStatus === 'saving' ? 'Mise à jour...' : t('dashboard.settings.security.submit')}
            </button>
            <button type="button" onClick={() => setActiveSettingsTab('main')}
              className="px-8 py-3 rounded-xl border border-[var(--border-color)] text-brand-gray font-bold hover:bg-white/5 transition-all">
              Annuler
      ...
      {activeSettingsTab === 'notifications' && (
        <div className="space-y-4 max-w-xl">
          ...
          <button onClick={() => setActiveSettingsTab('main')} className="mt-4 bg-brand-mint text-[#0D1117] px-8 py-3 rounded-xl font-bold hover:scale-105 transition-all">
            Enregistrer
          </button>
        </div>
      )}
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'projects': return renderMissionsList();
        ...
        <AnimatePresence>
          {isSidebarOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setIsSidebarOpen(false)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 md:hidden" />
          )}
        </AnimatePresence>

        <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 border-r border-[var(--border-color)] bg-[var(--bg-primary)] md:bg-transparent transform transition-transform duration-300 ease-in-out p-6 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          <div className="flex justify-between items-center mb-8 md:hidden">
            <div className="text-brand-mint font-bold text-xl">LUCID</div>
            <button onClick={() => setIsSidebarOpen(false)} className="text-brand-gray" aria-label="Fermer"><X size={24} /></button>
          </div>
          <nav className="space-y-2 h-full flex flex-col">
            ...
            <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-400/10 transition-all mt-auto">
        ...
        <main className="flex-grow p-4 md:p-8 w-full overflow-x-hidden">
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 md:mb-12">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              <button onClick={() => setIsSidebarOpen(true)} className="md:hidden p-2 rounded-lg bg-white/5 border border-[var(--border-color)] text-brand-mint" aria-label="Menu">
                <LayoutDashboard size={20} />
              </button>
              <div>
                <h1 className="text-2xl md:text-3xl font-bold mb-1">{t('dashboard.welcome').replace('{{name}}', profile?.first_name ?? '')}</h1>
      ...
      <AnimatePresence>
        {selectedMission && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedMission(null)} className="absolute inset-0 bg-black/80 backdrop-blur-sm" />
              ...
              <div className="p-8">
                <div className="flex justify-between items-start mb-8">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-brand-mint/10 flex items-center justify-center text-brand-mint font-bold text-xl">{selectedMission.company.charAt(0)}</div>
                    <div>
                  ...
                  <button onClick={() => setSelectedMission(null)} className="p-2 rounded-full hover:bg-white/5 text-brand-gray" aria-label="Fermer"><X size={24} /></button>
                </div>
                <div className="grid grid-cols-3 gap-4 mb-8">
                  {[
                    { icon: <Calendar size={14} />, label: 'Durée', value: selectedMission.duration },
                    { icon: <DollarSign size={14} />, label: 'Budget', value: selectedMission.budget },
                    ...
                    <div key={i} className="bg-[var(--bg-primary)] p-4 rounded-2xl border border-[var(--border-color)]">
                      <div className="flex items-center gap-2 text-brand-gray text-xs mb-1">{item.icon} {item.label}</div>
                      <div className="font-bold text-sm">{item.value}</div>
                    </div>
                  ))}
                </div>
                <p className="text-brand-gray leading-relaxed mb-8">{selectedMission.description}</p>
                <div className="flex gap-4">
                  {appliedMissions.includes(selectedMission.id) ? (
                    <div className="flex-grow bg-brand-mint/10 text-brand-mint py-4 rounded-xl font-bold text-center border border-brand-mint/20">Candidature envoyée</div>
                  ) : (
                    <button onClick={() => handleApply(selectedMission.id)} className="flex-grow bg-brand-mint text-[#0D1117] py-4 rounded-xl font-bold hover:scale-[1.02] transition-all">
                      Postuler à cette mission
                    </button>
                  )}
                  <button onClick={() => handleSave(selectedMission.id)}
