      ...
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <div className="flex items-center gap-4 ml-4 border-l border-[var(--border-color)] pl-8">
            <button onClick={toggleTheme} className="p-2 text-brand-gray hover:text-brand-mint transition-colors" aria-label="Changer le thème">
        ...
        <div className="flex items-center gap-4 md:hidden">
          ...
          <button onClick={() => setIsOpen(!isOpen)} aria-label="Menu" className="text-[var(--text-primary)]">
        ...
        {isOpen && (
          <motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className="flex justify-between items-center p-6 border-b border-white/5">
              ...
              <button onClick={() => setIsOpen(false)} aria-label="Fermer" className="text-white p-2">
                <X size={32} />
              </button>
            </div>
            ...
            <div className="flex-grow flex flex-col justify-center px-10 gap-8">
              {navLinks.map((link, i) => (
                <motion.a key={link.name} href={link.href}
                  ...
                  transition={{ delay: i * 0.1 }} onClick={() => setIsOpen(false)}
              ...
              {profile && (
                <motion.a href={profile.role === 'admin' ? '/admin' : '/dashboard'}
                  initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: navLinks.length * 0.1 }} onClick={() => setIsOpen(false)}
