    ...
    <section id="contact" className="py-24 bg-brand-anthracite">
      <div className="container mx-auto px-6 max-w-4xl">
        <div className="text-center mb-16">
          ...
          <p className="text-brand-mint font-bold tracking-wide uppercase text-sm">{t('contact.reassurance')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="space-y-2">
              ...
              <input type="text" name="company" value={form.company} onChange={handleChange}
                ...
                placeholder="Ex: Lucid Agency" maxLength={100} />
          ...
          <div className="space-y-2 mb-6">
            ...
            <input type="email" name="email" value={form.email} onChange={handleChange}
              ...
              placeholder="jean@société.com" maxLength={255} autoComplete="email" />
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <div className="space-y-2">
              ...
              <select name="type" value={form.type} onChange={handleChange} aria-label={t('contact.type')}
                ...
                {['Logo / Branding', 'Site Web', 'Vidéo', 'Stratégie', 'Autre'].map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-brand-gray ml-1">{t('contact.budget')}</label>
              <select name="budget" value={form.budget} onChange={handleChange} aria-label={t('contact.budget')}
                className="w-full bg-brand-anthracite border border-white/10 rounded-xl px-4 py-4 text-white focus:border-brand-mint outline-none transition-colors appearance-none">
                {['< 500K FCFA', '500K – 1M FCFA', '1M – 3M FCFA', '+3M FCFA'].map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-8">
            ...
            <textarea name="message" value={form.message} onChange={handleChange} rows={4} maxLength={5000}
            ...
            <div className="flex justify-between">
              {errors.message ? <p className="text-red-400 text-xs ml-1">{errors.message}</p> : <span />}
