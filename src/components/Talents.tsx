...
const talents = [
  ...
  { name: 'Amina L.', category: 'Web', expertise: 'Stratège Social Media', skills: ['Growth', 'Content Strategy', 'Ads'], initials: 'AL', color: '#6C63FF', rate: '65 000 FCFA/j', bio: 'Experte en croissance digitale. A piloté des campagnes pour 40+ marques avec un ROI moyen de 3x.' },
  { name: 'Kwame O.', category: 'Video', expertise: 'Motion Designer', skills: ['3D Animation', 'VFX', 'Storyboarding'], initials: 'KO', color: '#FF6B6B', rate: '90 000 FCFA/j', bio: 'Motion designer primé. Crée des vidéos qui captivent et convertissent pour les réseaux sociaux et la publicité.' },
  { name: 'Sofia M.', category: 'Web', expertise: 'Dev No-Code', skills: ['Webflow', 'Bubble', 'Automation'], initials: 'SM', color: '#FFB347', rate: '70 000 FCFA/j', bio: 'Développeuse no-code certifiée Webflow. Livre des sites performants en 2x moins de temps.' },
  { name: 'Léo B.', category: 'Photo', expertise: 'SEO & Data', skills: ['Audit Technique', 'Analytics', 'Ranking'], initials: 'LB', color: '#4ECDC4', rate: '75 000 FCFA/j', bio: 'Expert SEO avec un track record de +150% de trafic organique sur ses projets en 6 mois.' },
  { name: 'Yasmine D.', category: 'Branding', expertise: 'Copywriter', skills: ['Storytelling', 'UX Writing', 'Conversion'], initials: 'YD', color: '#A8E063', rate: '60 000 FCFA/j', bio: 'Copywriter spécialisée en conversion. Ses textes ont généré +200% de leads pour ses clients.' },
  { name: 'Marc F.', category: 'Video', expertise: 'Réalisateur', skills: ['Publicité', 'Documentaire', 'Montage'], initials: 'MF', color: '#FF8C94', rate: '95 000 FCFA/j', bio: 'Réalisateur publicitaire avec 10 ans d\'expérience. A travaillé pour des marques internationales.' },
  { name: 'Elena R.', category: 'Photo', expertise: 'Photographe', skills: ['Mode', 'Produit', 'Retouche'], initials: 'ER', color: '#C3B1E1', rate: '55 000 FCFA/j', bio: 'Photographe créative spécialisée produit et mode. Son travail a été publié dans plusieurs magazines.' },
  { name: 'Lucas P.', category: 'Branding', expertise: 'Brand Designer', skills: ['Logotype', 'Typographie', 'Identité'], initials: 'LP', color: '#00D2FF', rate: '80 000 FCFA/j', bio: 'Brand designer avec une approche data-driven. Crée des identités visuelles mémorables et cohérentes.' },
  { name: 'Sarah T.', category: 'Web', expertise: 'UX Designer', skills: ['User Research', 'Prototypage', 'Figma'], initials: 'ST', color: '#FFA07A', rate: '72 000 FCFA/j', bio: 'UX designer centrée utilisateur. Améliore les taux de conversion grâce à des interfaces intuitives.' },
  { name: 'David N.', category: 'Video', expertise: 'VFX Artist', skills: ['Compositing', '3D', 'Effets Spéciaux'], initials: 'DN', color: '#98FB98', rate: '#88000 FCFA/j', bio: 'Artiste VFX passionné. Crée des effets visuels spectaculaires pour la publicité et le cinéma.' },
  { name: 'Chloé A.', category: 'Photo', expertise: 'Directrice Photo', skills: ['Lumière', 'Portrait', 'Direction Artistique'], initials: 'CA', color: '#DDA0DD', rate: '85 000 FCFA/j', bio: 'Directrice photo avec un œil artistique unique. Transforme chaque projet en œuvre visuelle forte.' },
];
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-4xl mx-auto mb-16 space-y-6">
          <div className="relative group">
            ...
            />
            {searchQuery && (
              <button onClick={() => { setSearchQuery(''); setVisibleCount(6); }}
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {categories.map(({ key, label }) => (
              <button key={key} onClick={() => { setFilter(key); setVisibleCount(6); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          <AnimatePresence mode="popLayout">
            {displayedTalents.length > 0 ? displayedTalents.map((talent, i) => (
              <motion.div key={talent.name} layout
                ...
                <div className="h-32 relative flex items-center justify-center"
                  style={{ background: `linear-gradient(135deg, ${talent.color}15, ${talent.color}05)` }}>
                ...
                <div className="p-6">
                  <h3 className="text-xl font-bold mb-1 text-[var(--text-primary)]">{talent.name}</h3>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setSelectedTalent(talent)}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="flex items-center gap-5 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-[var(--text-primary)]">{selectedTalent.name}</h2>
                  <p className="font-medium" style={{ color: selectedTalent.color }}>{selectedTalent.expertise}</p>
              ...
              {/* Compétences */}
              <div className="mb-6">
                <p className="text-xs font-bold uppercase tracking-widest text-brand-gray mb-3">Compétences</p>
                <div className="flex flex-wrap gap-2">
                  {selectedTalent.skills.map(skill => (
                    <span key={skill}
                      className="text-xs font-bold px-3 py-1.5 rounded-full border"
                      style={{ color: selectedTalent.color, borderColor: `${selectedTalent.color}40`, background: `${selectedTalent.color}10` }}>
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-brand-mint/5 border border-brand-mint/20 mb-8">
                <span className="text-sm text-brand-gray font-medium">Taux journalier indicatif</span>
                <span className="font-bold text-brand-mint">{selectedTalent.rate}</span>
              </div>

              {/* CTA */}
              <a href="#contact" onClick={() => setSelectedTalent(null)}
